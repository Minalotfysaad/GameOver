...
export class Ui {
            ...
            content += `
                <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                    <div class="card p-2" data-id="${game.id}">
                        <img class="w-100" src="${game.thumbnail}" class="card-img-top rounded-1" alt="">
                        <div class="card-body">
                            ...
                            <p class="card-text">${game.short_description.split(" ").slice(0, 5).join(" ") + "..."}</p>
                        </div>
                        <div class="card-footer d-flex justify-content-between align-items-center text-center">
                            <span class="tag text-uppercase">${game.genre}</span>
    ...
    displayDetails(details) {
        const shortDescription = details.description.split(" ").slice(0, 100).join(" ");
        ...
        document.querySelector(".see-more").addEventListener("click", function() {
            ...
            this.previousElementSibling.style.display = fullDescription.classList.contains('d-none') ? 'inline' : 'none';
            this.style.display = fullDescription.classList.contains('d-none') ? 'inline' : 'none';
        });
